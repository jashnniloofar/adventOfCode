...
function nextTurn(playerHp, bossHp, mana, usedMana, shield, poison, recharge) {
    ...
    if (recharge > 0) mana = mana + Math.min(recharge, 2) * 101;

    poison = Math.max(0, poison - 2);
    shield = Math.max(0, shield - 2);
    recharge = Math.max(0, recharge - 2);
    //Magic Missile
    nextTurn(playerHp - 1, bossHp - 4, mana - 53, usedMana + 53, shield, poison, recharge);
    //Drain
    nextTurn(playerHp + 1, bossHp - 2, mana - 73, usedMana + 73, shield, poison, recharge);
    if (shield === 0) nextTurn(playerHp - 1, bossHp, mana - 113, usedMana + 113, 6, poison, recharge);
    //Poison
    if (poison === 0) nextTurn(playerHp - 1, bossHp - 3, mana - 173, usedMana + 173, shield, 5, recharge);
    //Recharge
    if (recharge === 0) nextTurn(playerHp - 1, bossHp, mana - 229, usedMana + 229, shield, poison, 5);
}
